...
import type { NsGraph, HookHub, NsEdgeCmd, NsNodeCmd, NsGroupCmd, IHooks,IArgsBase} from '@antv/xflow'
import type {IContext} from '@antv/xflow-core/es/command/interface'
...
type ICommand = ICommandHandler<NsGraphRenderCustom.IArgs, NsGraphRenderCustom.IResult, NsGraphRenderCustom.ICmdHooks>
...
export namespace NsGraphRenderCustom {
  /** Command: 用于注册named factory */
  export const command = CustomCommands.RENDER_GRAPH_CUSTOM
  /** hookName */
  export const hookKey = 'renderGraphCustom'
...
@injectable({
  token: { token: ICommandHandler, named: NsGraphRenderCustom.command.id },
})
/** 画布渲染命令 */
export class GraphRenderCustomCommand implements ICommand {
  @inject(ICommandContextProvider) contextProvider: ICommand['contextProvider']

  ctx: IContext<NsGraphRenderCustom.IArgs, NsGraphRenderCustom.IResult, NsGraphRenderCustom.ICmdHooks>
  ...
  execute = async () => {
    ...
    const result = await hooks.graphRender.call(
      ...
      async handlerArgs => {
        ...
        const { beforeRender, graphData, isNodeEqual, isEdgeEqual, afterRender, zoomParams } = handlerArgs

        /** 如果用户自定义beforeRender方法 */
        beforeRender && beforeRender(graphMeta)

        await this.doLoadGraph(x6Graph, graphData, zoomParams, isNodeEqual, isEdgeEqual)
  ...
  private doLoadGraph = async (
    ...
    const graphScale = x6Graph.transform.getScale()
    const graphTranslation = x6Graph.transform.getTranslation()
    let sx = 1
    let sy = 1
    let tx = 0
    let ty = 0
    if(zoomParams && zoomParams.scale){
        sx = zoomParams.scale.sx 
        sy = zoomParams.scale.sy
    }
    if(zoomParams && zoomParams.translate){
        tx = zoomParams.translate.tx 
        ty = zoomParams.translate.ty
    }
    if(sx === graphScale.sx && sy === graphScale.sy){
        //
    }
    else {
        x6Graph.scale(sx, sy)
    }
    if(tx === graphTranslation.tx && ty === graphTranslation.ty){
        //
    }
    else {
        x6Graph.translate(tx, ty)
    }
    if (x6Graph?.isFrozen()) {
      x6Graph?.unfreeze()
    }
